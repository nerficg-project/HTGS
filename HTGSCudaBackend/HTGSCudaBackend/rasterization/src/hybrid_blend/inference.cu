...
void blend_k_templated(
    ...
    Args&&... kernel_args)
{
    if (K >= 32) htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<32><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 16) htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<16><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 8) htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<8><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 4) htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<4><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 2) htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<2><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else htgs::rasterization::hybrid_blend::kernels::inference::blend_cu<1><<<grid, block>>>(std::forward<Args>(kernel_args)...);
}
