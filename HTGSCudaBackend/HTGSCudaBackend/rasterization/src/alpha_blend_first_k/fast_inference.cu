...
void blend_k_templated(
    ...
    Args&&... kernel_args)
{
    if (K >= 32) htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<32><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 16) htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<16><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 8) htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<8><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 4) htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<4><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 2) htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<2><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else htgs::rasterization::alpha_blend_first_k::kernels::fast_inference::blend_cu<1><<<grid, block>>>(std::forward<Args>(kernel_args)...);
}

void htgs::rasterization::alpha_blend_first_k::fast_inference(
